...
const { hookUsesRegistryNpx } = require('./hooks');
...
function checkBrowser(projectRoot, config) {
  ...
  if (executable) {
    ...
    hint = 'The browser for this Playwright version is not downloaded, which is normal after an update. Run: npx --no playwright install chromium';
  } else {
...
function checkConfig(projectRoot, config) {
  ...
  if (!fs.existsSync(configPath)) {
    return [fail('Config', `${VALIDATOR_DIR}/${CONFIG_FILENAME} not found. Run: npx --no hugo-validator init`)];
  }
...
function checkHook(projectRoot) {
  ...
  return judgeHook(hook, hooksPath);
}

/** Verdict on a pre-commit hook's text */
function judgeHook(hook, hooksPath) {
  if (!hook) return warn('Git hook', `no pre-commit hook in ${hooksPath}. Run: npx --no hugo-validator setup-hooks`);
  if (!/hugo-validator/.test(hook)) {
    return warn('Git hook', `${hooksPath}/pre-commit does not call hugo-validator. Run: npx --no hugo-validator migrate`);
  }
  if (hookUsesRegistryNpx(hook)) {
    // Hooks from older versions. Without a local install, the package runner asks the npm
    // registry for "hugo-validator", which this project does not own.
    return warn('Git hook', `${hooksPath}/pre-commit was generated by an older version and could fetch a package named ` +
      'hugo-validator from the npm registry. Regenerate it: npx --no hugo-validator setup-hooks --force');
  }
  return ok('Git hook', `${hooksPath}/pre-commit runs the locally installed hugo-validator`);
}
...
function checkLegacy(projectRoot) {
  ...
  return legacy.length === 0
    ...
    : warn('Legacy files', `${legacy.map(item => item.path).join(', ')}. Run: npx --no hugo-validator migrate`);
}
...
module.exports = { doctor, judgeHook, parseVersion, versionAtLeast, satisfiesCaret };
