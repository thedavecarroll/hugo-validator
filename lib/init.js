...
module.exports = {
  ...
  // Test server settings (read by hugo-validator/playwright.config.ts)
  testServerPort: 3000,
  // testServerCommand: 'npx serve -l 3000 ../public', // default: python3 -m http.server
};
`;
}
...
// Port and server command come from hugo-validator.config.js
const requireFromProject = createRequire(path.join(process.cwd(), 'hugo-validator', 'playwright.config.ts'));
let userConfig: { testServerPort?: number; testServerCommand?: string | null } = {};
try {
  userConfig = requireFromProject('./hugo-validator.config.js');
} catch {
  // No config file - use defaults
}

const PORT = userConfig.testServerPort || 3000;
...
export default defineConfig({
  ...
  webServer: {
    command: userConfig.testServerCommand || \`python3 -m http.server \${PORT} --directory ../public\`,
    url: BASE_URL,
...
/**
 * Refresh hugo-validator/tests/ from the installed package.
 * Touches nothing else: config, hooks and linting configs are left alone.
 */
async function updateTests() {
  const validatorDir = path.join(process.cwd(), VALIDATOR_DIR);
  if (!fs.existsSync(validatorDir)) {
    throw new Error(`${VALIDATOR_DIR}/ not found - run 'npx hugo-validator init' first`);
  }
  await copyTests(validatorDir, { force: true });
}

module.exports = { init, updateTests, copyTests };
