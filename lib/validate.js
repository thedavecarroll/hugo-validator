...
const { prepareRuntime, prepareHtmlValidateConfig, PACKAGE_TESTS_DIR } = require('./runtime');
const { resolveBin, resolvePlaywright, toolEnv, PACKAGE_ROOT } = require('./tools');

/**
 * Run one of the package's tools with Node, from the site root.
 * Returns a spawnSync-shaped result. A missing tool becomes result.error.
 */
function runTool(packageName, binName, args) {
  let tool;
  try {
    tool = resolveBin(packageName, binName);
  } catch (error) {
    return { error, status: null, stdout: '', stderr: '' };
  }
  return spawnSync(process.execPath, [tool.bin, ...args], { encoding: 'utf8', env: toolEnv() });
}
...
function walkFiles(target, root = process.cwd()) {
  ...
  const toRelative = (file) => {
    ...
    if (file.startsWith(PACKAGE_ROOT + path.sep)) {
      return `<hugo-validator>/${path.relative(PACKAGE_ROOT, file).split(path.sep).join('/')}`;
    }
...
function getStageInputs(config) {
  ...
  return {
    ...
    css: [
      ...
      { path: path.join(PACKAGE_ROOT, 'configs', 'stylelint.base.json') },
    ],
    html: [
      { path: 'public', match: '**/*.html' },
      { path: 'hugo-validator/.htmlvalidate.json' },
      { path: path.join(PACKAGE_ROOT, 'configs', 'htmlvalidate.base.js') },
...
async function runCssValidation(config, reportDir, saveLog) {
  ...
  const result = runTool('stylelint', 'stylelint', [
    '--config', 'hugo-validator/.stylelintrc.json', '--formatter', 'verbose', '--allow-empty-input', config.cssPattern,
  ]);
  const output = result.error
...
function listHtmlFiles(config, root = process.cwd()) {
  ...
  return walkFiles('public', root)
    ...
    .filter(file => !isHugoRedirectPage(path.resolve(root, file)));
}

/**
 * Hugo writes a stub page with a meta refresh for every alias and for
 * /page/1/. Hugo generates them, they have no <body>, and the site author
 * cannot fix them, so they are not validated. The Playwright tests skip them
 * the same way (listSitePages in tests/helpers.ts).
 */
function isHugoRedirectPage(file) {
  try {
    const fd = fs.openSync(file, 'r');
    try {
      const buffer = Buffer.alloc(1024);
      const read = fs.readSync(fd, buffer, 0, 1024, 0);
      return /http-equiv=["']?refresh/i.test(buffer.toString('utf8', 0, read));
    } finally {
      fs.closeSync(fd);
    }
  } catch {
...
async function runHtmlValidation(config, reportDir, saveLog) {
  ...
  // The site's config with the package's base merged in (see lib/runtime.js)
  let htmlValidateConfig;
  try {
    htmlValidateConfig = prepareHtmlValidateConfig();
  } catch (error) {
    const message = `Could not read hugo-validator/.htmlvalidate.json: ${error.message}`;
    console.log('❌ HTML validation failed');
    console.log(message);
    writeStageLog(reportDir, saveLog, 'html-validation.log', message, true);
  ...
  for (let i = 0; i < files.length; i += CHUNK_SIZE) {
    ...
    const result = runTool('html-validate', 'html-validate', [
      '--config', htmlValidateConfig, '--formatter', 'stylish', ...chunk,
    ]);
    if (result.error) {
...
async function runPlaywrightTests(config, reportDir, saveLog, lastFailedOnly = false, verbose = false) {
  return new Promise((resolve) => {
    ...
    // Same Playwright copy for the runner and the tests, launched without npx
    const playwrightCli = resolvePlaywright(runtime.testsDir);
    const args = [playwrightCli.bin, 'test', '--config', runtime.configArg];
    ...
    const playwright = spawn(process.execPath, args, {
      stdio: ['pipe', 'pipe', 'pipe'],
      env: toolEnv(process.env, { HUGO_VALIDATOR_VERBOSE: verbose ? '1' : '0' }),
    });
...
async function runTests(filters = [], options = {}) {
  ...
  const runtime = prepareRuntime(config);
  const playwrightCli = resolvePlaywright(runtime.testsDir);
  const args = [playwrightCli.bin, 'test', '--config', runtime.configArg, ...filters];
  if (options.ui) args.push('--ui');
  if (!process.env.CI && config.testWorkers && !options.ui) args.push(`--workers=${config.testWorkers}`);

  return new Promise((resolve) => {
    const child = spawn(process.execPath, args, { stdio: 'inherit', env: toolEnv() });
