...
function getStageInputs(config) {
  ...
  return {
    ...
    tests: [
      { path: 'public' },
      { path: 'hugo-validator/tests' },
      { path: 'hugo-validator/playwright.config.ts' },
      { path: validatorConfig },
    ],
  };
}
 ...
 */
function computeDigest(inputs, root = process.cwd()) {
  const seen = new Set();
...
async function validate(options = {}) {
  ...
  for (const stage of stages) {
    ...
    const digest = computeDigest(stageInputs[stage]);
...
async function killPorts(ports) {
  ...
  for (const port of ports || []) {
    ...
    let pids = [];
    try {
      const output = execSync(`lsof -ti tcp:${portNum} -sTCP:LISTEN`, {
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'ignore'],
      });
      pids = output.split('\n').map(line => parseInt(line, 10)).filter(pid => pid > 0 && pid !== process.pid);
    } catch {
      continue; // lsof exits non-zero when nothing listens on the port
    }
